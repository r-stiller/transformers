...
class Message:
    ...
    def time(self) -> str:
        time_spent = [
            r["time_spent"].split(", ")[0] for r in [*self.model_results.values(), *self.additional_results.values()]
        ]
        total_secs = 0
    ...
    def model_failures(self) -> Dict:
        ...
        for key, value in failures.items():
            if "single" in value and "multi" in value:
                device_report = f"{str(value['single']).rjust(3)}/{str(value['multi']).rjust(3)}"
            elif "single" in value:
                device_report = f"{str(value['single']).rjust(3)}/{'0'.rjust(3)}"
            elif "multi" in value:
                device_report = f"{'0'.rjust(3)}/{str(value['multi']).rjust(3)}"
            else:
                device_report = None
        ...
        return {
            ...
            "text": {
                ...
                f"The following non-model modules had failures [single/multi]:\n{module_failures_report}",
            },
        }
    ...
    def post(self):
        ...
        #     channel=os.environ["CI_SLACK_CHANNEL_DUMMY_TESTS"],
        #     blocks=self.payload,
        #     text=f"{self.n_failures} failures out of {self.n_tests} tests,"
        #     if self.n_failures
        ...
        for job, job_result in self.model_results.items():
            if len(job_result["failures"]):
                for device, failures in job_result["failures"].items():
                    text = "\n".join(
                        sorted(
                            [f"*{k}*: {sum(v.values())}" for k, v in job_result["failed"].items() if sum(v.values())]
                        )
                    )
        ...
        models = [
            ...
            for a in list(
                filter(
                    os.path.isdir,
                    (
                        os.path.join("/home/lysandre/transformers/tests", f)
                        for f in os.listdir("/home/lysandre/transformers/tests")
                    ),
                )
            )
        ]
